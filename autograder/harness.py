...
def truncate_output(output):
    if output is not None and len(output) > 50000:
...
class AutograderHarness(object):
    ...
    def _load_payload(self):
        ...
        self.timeout_seconds = int(os.getenv("TIMEOUT_SECONDS") or 1200)
    ...
    #     response = requests.post(USER_SESSION_URL, data={"email": "admin@example.com", "password": "password" })
    ...
    def write_metadata(self):
        with open('/autograder/submission_metadata.json', 'w') as f:
            metadata = {"users": self.submission_json['active_users'],
    ...
    def run_autograder(self):
        ...
        with open('/autograder/results/stdout', 'w+') as stdoutf:
            ...
            return_code, elapsed_time, timed_out = timeout_process(p, self.timeout_seconds)
    ...
    def read_results(self):
        ...
        with codecs.open(results_path, 'r', encoding='utf-8', errors='replace') as f:
            contents = f.read()
            try:
                results = json.loads(clean_string(contents))
            except ValueError:
                return contents
